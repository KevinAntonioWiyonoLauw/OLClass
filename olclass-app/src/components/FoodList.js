...
// Komponen FoodList dioptimalkan dengan memo untuk mencegah render ulang yang tidak perlu
const FoodList = memo(({ searchQuery = '', foods = [], toggleFavorite, favorites = [], isFavorites = false }) => {
    // Mengambil fungsi setFoods dari konteks FoodContext
    const { setFoods } = useContext(FoodContext);
    // State untuk makanan yang dipilih
    ...
    // Mengambil data makanan saat komponen dimount atau saat searchQuery berubah
    useEffect(() => {
        if (!isFavorites) {
    ...
    // Membuka modal dan mengambil detail resep
    ...
    // Pengaturan untuk komponen Slider berdasarkan apakah sedang menampilkan favorit atau tidak
    ...
    return (
        <div className="p-6 relative">
            {/* Menampilkan gradasi transparan jika bukan halaman favorit */}
            {!isFavorites && (
                <>
            ...
            {/* Komponen Slider untuk menampilkan daftar makanan */}
            <Slider {...settings}>
                {foods.length > 0 ? (
                    foods.map(food => (
                    ...
                    // Menampilkan pesan jika tidak ada resep ditemukan
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-lg w-full p-6 relative overflow-y-auto max-h-[80vh] transform transition-transform duration-300 scale-100">
                        ...
                        {isLoading ? (
                            ...
                            // Menampilkan detail resep jika data berhasil diambil
                            <>
                                <h2
                                ...
                                {recipeDetails.images && recipeDetails.images.length > 0 ? (
                                    ...
                                    // Menampilkan gambar jika tidak ada multiple images
                                    <img
                                        className="w-full h-48 object-cover rounded mb-4"
                            ...
                            // Menampilkan pesan jika tidak ada detail tersedia
